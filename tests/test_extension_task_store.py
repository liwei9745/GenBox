...
window.extensionPrimaryAction();if(!element('extPassword').focused)throw new Error('credential guide action did not focus the password field');element('extPassword').value='session-only';window.extensionCredentialChanged();if(element('extGuidePrimaryBtn').textContent!=='extensions.personal_check_continue')throw new Error('credential state did not offer the read-only check');
let visited=[];const originalNext=window.extensionNext;window.extensionNext=step=>{visited.push(step);return originalNext(step)};await window.extensionPrimaryAction();if(discoverCalls!==1||sshCalls!==0||visited.at(-1)!==2)throw new Error('read-only discovery did not advance without a deployment diagnostic');
})();
'''
    result = subprocess.run(["node", "-e", node, str(source)], text=True, capture_output=True)
    assert result.returncode == 0, result.stderr
