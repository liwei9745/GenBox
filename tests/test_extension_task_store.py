...
def test_host_key_mismatch_requires_explicit_local_reset_before_new_pairing_in_node():
    source = Path(__file__).parents[1] / "static" / "js" / "extensions.js"
    node = r'''
const fs=require('fs');const source=fs.readFileSync(process.argv[1],'utf8');
(async()=>{
const elements=new Map();function element(id){if(!elements.has(id)){const classes=new Set(['extNetworkHostKey','extHostKeyConfirm','extHostKeyPairingCommandWrap'].includes(id)?['hidden']:[]);elements.set(id,{id,style:{},value:'',textContent:'',innerHTML:'',disabled:false,checked:false,dataset:{},options:[],selectedIndex:0,classList:{toggle(n,on){if(on)classes.add(n);else classes.delete(n)},add(n){classes.add(n)},remove(n){classes.delete(n)},contains(n){return classes.has(n)}},querySelector(){return element('nested')},querySelectorAll(){return []},focus(){this.focused=true},setAttribute(){},removeAttribute(){},closest(){return null}})}return elements.get(id)}
const network={value:'tailscale',checked:true,classList:{toggle(){}}};global.window=global;global.document={getElementById:element,querySelector(s){if(s.includes('extNetwork'))return network;return element('query')},querySelectorAll(){return []},addEventListener(){},removeEventListener(){}};global.i18nText=k=>k;global.getUiLanguage=()=> 'en';global.escHtml=v=>String(v||'');global.clearInterval=()=>{};global.setInterval=()=>({});global.localStorage=new Proxy({}, {get(){throw new Error('host recovery touched browser storage')},set(){throw new Error('host recovery touched browser storage')}});global.sessionStorage=new Proxy({}, {get(){throw new Error('host recovery touched browser storage')},set(){throw new Error('host recovery touched browser storage')}});
const key='SHA256:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';let target={id:'saved',name:'Saved',host:'safe.example',port:22,username:'user',host_key_algorithm:'ssh-ed25519',host_key:key,chatgpt2api_port:33010};let discoverCalls=0,pairStarts=0,probeCalls=0,resetBodies=[];global._authFetch=async(url,options={})=>{let body={};if(url==='/api/extensions/targets')body={targets:[target]};else if(url==='/api/extensions/catalog')body={categories:[],items:[]};else if(url==='/api/extensions/targets/batch')body={target_ids:[]};else if(url==='/api/extensions/tasks')body={tasks:[]};else if(url==='/api/extensions/ssh/test')body={ok:true,privileges:{is_root:true,can_deploy:true}};else if(url==='/api/extensions/discover'){discoverCalls+=1;return {ok:false,text:async()=>JSON.stringify({detail:{error:'hidden',diagnostic:{code:'ssh_host_key_mismatch',stage:'host_key_verification'}}})}}else if(url==='/api/extensions/ssh/host-key/reset'){resetBodies.push(JSON.parse(options.body));target={...target,host_key_algorithm:'',host_key:'',identity_version:2,available_networks:[],network_url:'',network_verified_at:''};body={target,reset:true}}else if(url==='/api/extensions/ssh/host-key/pair/start'){pairStarts+=1;body={pairing_id:'new-pairing',expires_at:Date.now()/1000+30,helper_command:'printf pairing'}}else if(url==='/api/extensions/ssh/host-key/probe'){probeCalls+=1;body={}}return {ok:true,text:async()=>JSON.stringify(body)}};
eval(source);window.extensionLoadServices=async()=>{};await window.loadExtensions();window.extensionLoadTarget('saved');element('extPassword').value='session-only-secret';window.extensionCredentialChanged();await window.extensionTestSSH(false);window.extensionNext(2);await window.extensionDiscover();
if(discoverCalls!==1)throw new Error('discovery was not attempted exactly once');if(element('extPassword').value||element('extPrivateKey').value||element('extSudoPassword').value)throw new Error('mismatch retained session credentials');if(pairStarts||probeCalls)throw new Error('mismatch automatically started identity confirmation');if(element('extGuidePrimaryBtn').textContent!=='extensions.identity_reset_action')throw new Error('recovery guide did not require an explicit reset');await window.extensionStartHostKeyPairing();await window.extensionStartHostKeySetup();if(pairStarts||probeCalls)throw new Error('recovery state allowed pairing or manual probing before reset');
global.window.confirm=()=>false;await window.extensionResetHostKey();if(resetBodies.length)throw new Error('cancelled confirmation reset host trust');global.window.confirm=()=>true;await window.extensionResetHostKey();if(JSON.stringify(resetBodies)!==JSON.stringify([{target_id:'saved'}]))throw new Error('reset request exposed more than target id');if(element('extensionMessage').textContent.includes('session-only-secret'))throw new Error('recovery message exposed a session credential');if(element('extGuidePrimaryBtn').textContent!=='extensions.host_key_pairing_start')throw new Error('reset did not return to the normal identity-confirmation entry');await window.extensionStartHostKeyPairing();if(pairStarts!==1||probeCalls!==0)throw new Error('new pairing was not allowed only after explicit reset');
})();
'''
    result = subprocess.run(["node", "-e", node, str(source)], text=True, capture_output=True)
    assert result.returncode == 0, result.stderr
...
const response='GENBOX-PAIR/1 code=challenge_identifier_12345 proof='+'a'.repeat(64);
global._authFetch=async(url,options={})=>{let body={};if(url==='/api/extensions/targets')body={targets:[target]};else if(url==='/api/extensions/catalog')body={categories:[],items:[]};else if(url==='/api/extensions/targets/batch')body={target_ids:[]};else if(url==='/api/extensions/tasks')body={tasks:[]};else if(url==='/api/extensions/ssh/host-key/pair/start'){startCalls+=1;body={pairing_id:'pairing_identifier_12345',expires_at:Date.now()/1000+30,helper_command:'printf pairing'};}else if(url==='/api/extensions/ssh/host-key/pair/complete'){completeCalls+=1;const sent=JSON.parse(options.body);if(sent.response!==response)throw new Error('confirmation code was not forwarded exactly once');return await new Promise(resolve=>{releaseComplete=()=>resolve({ok:true,text:async()=>JSON.stringify({target:{...target,host_key_algorithm:'ssh-ed25519',host_key:key}})})})}return {ok:true,text:async()=>JSON.stringify(body)}};
eval(source);window.extensionLoadServices=async()=>{};await window.loadExtensions();window.extensionLoadTarget('saved');await window.extensionStartHostKeyPairing();if(startCalls!==1||element('extHostKeyPairingCommand').value!=='printf pairing')throw new Error('mock pairing command did not render');if(element('extHostKeyPairingCompleteBtn').disabled!==true||element('extHostKeyPairingResponse').disabled!==false)throw new Error('empty confirmation-code state was not enforced');element('extHostKeyPairingResponse').value=response;(listeners.input||[]).forEach(fn=>fn({target:element('extHostKeyPairingResponse')}));if(element('extHostKeyPairingResponse').value!==''||element('extHostKeyPairingCompleteBtn').disabled)throw new Error('confirmation code was not hidden and enabled');const pending=window.extensionCompleteHostKeyPairing();await Promise.resolve();if(completeCalls!==1||!element('extHostKeyPairingResponse').disabled||!element('extHostKeyPairingCopyBtn').disabled||!element('extHostKeyPairingCancelBtn').disabled)throw new Error('submit did not lock pairing controls');releaseComplete();await pending;if(element('extHostKeyPairingCommand').value!==''||element('extHostKeyPairingResponse').value!==''||!element('extHostKeyPairing').classList.contains('hidden'))throw new Error('successful pairing did not clear transient UI state');
})();
'''
    result = subprocess.run(["node", "-e", node, str(source)], text=True, capture_output=True)
    assert result.returncode == 0, result.stderr
...
eval(source);window.extensionLoadServices=async()=>{};await window.loadExtensions();window.extensionLoadTarget('saved');await window.extensionStartHostKeyPairing();if(element('extHostKeyPairingCommand').value!==''||!element('extHostKeyPairingCommandWrap').classList.contains('hidden')||!element('extHostKeyPairingState').classList.contains('error'))throw new Error('expired pairing did not clear command and show recovery');if(!element('extHostKeyPairingStartBtn').focused)throw new Error('expired pairing did not restore keyboard focus to restart');await window.extensionStartHostKeyPairing();if(starts!==2||element('extHostKeyPairingCommand').value!=='printf pairing-2'||element('extHostKeyPairingStartBtn').disabled!==true)throw new Error('expired pairing could not be restarted');
})();
'''
    result = subprocess.run(["node", "-e", node, str(source)], text=True, capture_output=True)
    assert result.returncode == 0, result.stderr
...
const key='SHA256:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';
const image='registry.example/chatgpt2api@sha256:'+('a'.repeat(64));
...
eval(source);window.extensionLoadServices=async()=>{};await window.loadExtensions();window.extensionLoadTarget('saved');element('extPassword').value='session-only';element('extImage').value=image;window.extensionCredentialChanged();await window.extensionTestSSH(false);window.extensionNext(2);
...
function boot(fetchImpl,cryptoImpl){
  ...
  const context={console,Uint8Array,Array,Promise,JSON,Number,String,Object,Math,Set,Map,Error,TypeError,RegExp,Date,AbortController,
    document:{getElementById:element,querySelector(){return element('query')},querySelectorAll(){return []},addEventListener(){},removeEventListener(){}},
        i18nText:k=>k,getUiLanguage:()=> 'en',escHtml:v=>String(v||''),_authFetch:fetchImpl,crypto:cryptoImpl,confirm:()=>true,
...
const context={console,Uint8Array,Array,Promise,JSON,Number,String,Object,Math,Set,Map,Error,TypeError,RegExp,Date,AbortController,confirm:()=>true,
  document:{getElementById:element,querySelector(){return element('query')},querySelectorAll(){return []},addEventListener(){},removeEventListener(){}},
...
if(!empty.checked||working.checked||!strategy.checked||element('extCloneSource').value!==''||element('extServicePort').value!=='33011'||element('extImage').value!=='registry.example/chatgpt2api@sha256:'+('a'.repeat(64)))throw new Error('snapshot recovery changed the deployment draft');
...
def test_novice_step_one_starts_server_confirmation_without_host_key_probe_in_node():
    source = Path(__file__).parents[1] / "static" / "js" / "extensions.js"
    node = r'''
const fs=require('fs');const source=fs.readFileSync(process.argv[1],'utf8');
(async()=>{
...
const target={id:'saved',name:'Saved',host:'safe.example',port:22,username:'user',host_key_algorithm:'',host_key:'',chatgpt2api_port:33010};let pairingStarts=0,probes=0,cancellations=[];
global._authFetch=async(url,options={})=>{let body={};if(url==='/api/extensions/targets')body={targets:[target]};else if(url==='/api/extensions/catalog')body={categories:[],items:[]};else if(url==='/api/extensions/targets/batch')body={target_ids:[]};else if(url==='/api/extensions/tasks')body={tasks:[]};else if(url==='/api/extensions/ssh/host-key/pair/start'){pairingStarts+=1;body={pairing_id:'local-pairing',expires_at:Date.now()/1000+30,helper_command:'printf local-confirmation'}}else if(url==='/api/extensions/ssh/host-key/pair/cancel'){cancellations.push(JSON.parse(options.body));body={cancelled:true}}else if(url==='/api/extensions/ssh/host-key/probe'){probes+=1;body={}}return {ok:true,text:async()=>JSON.stringify(body)}};
eval(source);window.extensionLoadServices=async()=>{};await window.loadExtensions();window.extensionLoadTarget('saved');
if(element('extGuidePrimaryBtn').textContent!=='extensions.host_key_pairing_start')throw new Error('saved target did not offer server confirmation');await window.extensionPrimaryAction();
if(pairingStarts!==1||probes!==0)throw new Error('novice confirmation did not use pairing-only start');if(element('extHostKeyPairingCommand').value!=='printf local-confirmation')throw new Error('pairing helper did not render');if(element('extGuidePrimaryBtn').textContent!=='status.processing'||!element('extGuidePrimaryBtn').disabled)throw new Error('guide did not wait for the confirmation-code steps');if(!element('extHostKeyPairingCopyBtn').focused)throw new Error('confirmation flow did not focus copy');await window.extensionCancelHostKeyPairing();if(cancellations.length!==1||JSON.stringify(cancellations[0])!==JSON.stringify({pairing_id:'local-pairing'}))throw new Error('cancel did not submit only the pairing id');if(probes!==0)throw new Error('cancel invoked a host-key probe');if(element('extGuidePrimaryBtn').textContent!=='extensions.host_key_pairing_start'||element('extGuidePrimaryBtn').disabled)throw new Error('cancel did not restore the restart action');if(!element('extHostKeyPairingStartBtn').focused)throw new Error('cancel did not restore keyboard focus to restart');await window.extensionPrimaryAction();window.extensionConnectionChanged();await Promise.resolve();await Promise.resolve();if(pairingStarts!==2||cancellations.length!==2||JSON.stringify(cancellations[1])!==JSON.stringify({pairing_id:'local-pairing'}))throw new Error('editing the target did not discard the active pairing');if(probes!==0)throw new Error('target editing invoked a host-key probe');
})();
'''
    result = subprocess.run(["node", "-e", node, str(source)], text=True, capture_output=True)
    assert result.returncode == 0, result.stderr
...
def test_novice_step_one_executes_save_host_key_credential_and_read_only_discovery_in_node():
    source = Path(__file__).parents[1] / "static" / "js" / "extensions.js"
    node = r'''
const fs=require('fs');const source=fs.readFileSync(process.argv[1],'utf8');
(async()=>{
...
if(element('extGuidePrimaryBtn').textContent!=='common.save')throw new Error('unsaved target did not offer save');await window.extensionPrimaryAction();if(saveCalls!==1||element('extGuidePrimaryBtn').textContent!=='extensions.host_key_pairing_start')throw new Error('save did not advance to server confirmation');
...
window.extensionPrimaryAction();if(!element('extPassword').focused)throw new Error('credential guide action did not focus the password field');element('extPassword').value='session-only';element('extElevation').value='password_sudo';element('extSudoPassword').value='must-not-send';element('extReuseSshPassword').checked=true;window.extensionCredentialChanged();if(element('extGuidePrimaryBtn').textContent!=='extensions.personal_check_continue')throw new Error('credential state did not offer the read-only check');
let visited=[];const originalNext=window.extensionNext;window.extensionNext=step=>{visited.push(step);return originalNext(step)};await window.extensionPrimaryAction();if(discoverCalls!==1||sshCalls!==0||visited.at(-1)!==2)throw new Error('read-only discovery did not advance without a deployment diagnostic');if(discoverBodies[0].credential.elevation!=='none'||discoverBodies[0].credential.sudo_password!==''||discoverBodies[0].credential.reuse_ssh_password!==false)throw new Error('read-only discovery sent deployment elevation inputs');
})();
'''
    result = subprocess.run(["node", "-e", node, str(source)], text=True, capture_output=True)
    assert result.returncode == 0, result.stderr
...
def test_read_only_discovery_without_deploy_capability_keeps_diagnostic_optional_in_node():
    source = Path(__file__).parents[1] / "static" / "js" / "extensions.js"
    node = r'''
const fs=require('fs');const source=fs.readFileSync(process.argv[1],'utf8');
(async()=>{
...
const network={value:'tailscale',checked:true,classList:{toggle(){}}};global.window=global;global.document={getElementById:element,querySelector(s){if(s.includes('extNetwork'))return network;return element('query')},querySelectorAll(){return []},addEventListener(){},removeEventListener(){}};global.i18nText=k=>k;global.getUiLanguage=()=> 'zh-CN';global.escHtml=v=>String(v||'');global.clearInterval=()=>{};global.setInterval=()=>({});
const key='SHA256:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';const target={id:'saved',name:'Saved',host:'vps.example',port:22,username:'deploy-user',host_key_algorithm:'ssh-ed25519',host_key:key,chatgpt2api_port:33010};let discoveryCalls=0,sshCalls=0;
...
def test_plan_generation_requires_explicit_read_only_recheck_confirmation_in_node():
    source = Path(__file__).parents[1] / "static" / "js" / "extensions.js"
    node = r'''
const fs=require('fs');const source=fs.readFileSync(process.argv[1],'utf8');
(async()=>{
const elements=new Map();function element(id){if(!elements.has(id)){const classes=new Set(['extDiscoveryResult','extPlanPreview'].includes(id)?['hidden']:[]);elements.set(id,{id,style:{},value:'',textContent:'',innerHTML:'',disabled:false,checked:false,dataset:{},options:[],selectedIndex:0,classList:{toggle(n,on){if(on)classes.add(n);else classes.delete(n)},add(n){classes.add(n)},remove(n){classes.delete(n)},contains(n){return classes.has(n)}},querySelector(){return element('nested')},querySelectorAll(){return []},focus(){},setAttribute(){},removeAttribute(){},closest(){return null}})}return elements.get(id)}
const network={value:'tailscale',checked:true,classList:{toggle(){}}};global.window=global;global.document={getElementById:element,querySelector(s){if(s.includes('extNetwork'))return network;return element('query')},querySelectorAll(){return []},addEventListener(){},removeEventListener(){}};global.i18nText=k=>k;global.getUiLanguage=()=> 'zh-CN';global.escHtml=v=>String(v||'');global.clearInterval=()=>{};global.setInterval=()=>({});
const key='SHA256:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';const target={id:'saved',name:'Saved',host:'safe.example',port:22,username:'deploy-user',host_key_algorithm:'ssh-ed25519',host_key:key,chatgpt2api_port:33010};let allowPlan=false,planCalls=[];
...
def test_cancelled_deployment_confirmation_keeps_the_plan_and_sends_no_deploy_request():
    source = Path(__file__).parents[1] / "static" / "js" / "extensions.js"
    node = r'''
const fs=require('fs');const source=fs.readFileSync(process.argv[1],'utf8');
(async()=>{
const elements=new Map();function element(id){if(!elements.has(id)){const classes=new Set(['extDiscoveryResult','extPlanPreview'].includes(id)?['hidden']:[]);elements.set(id,{id,style:{},value:'',textContent:'',innerHTML:'',disabled:false,checked:false,dataset:{},options:[],selectedIndex:0,classList:{toggle(n,on){if(on)classes.add(n);else classes.delete(n)},add(n){classes.add(n)},remove(n){classes.delete(n)},contains(n){return classes.has(n)}},querySelector(){return element('nested')},querySelectorAll(){return []},focus(){},setAttribute(){},removeAttribute(){},closest(){return null}})}return elements.get(id)}
const network={value:'tailscale',checked:true,classList:{toggle(){}}};global.window=global;let allowConfirmation=true;global.confirm=()=>allowConfirmation;global.document={getElementById:element,querySelector(s){if(s.includes('extNetwork'))return network;return element('query')},querySelectorAll(){return []},addEventListener(){},removeEventListener(){}};global.i18nText=k=>k;global.getUiLanguage=()=> 'zh-CN';global.escHtml=v=>String(v||'');global.clearInterval=()=>{};global.setInterval=()=>({});
const key='SHA256:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA';const target={id:'saved',name:'Saved',host:'safe.example',port:22,username:'deploy-user',host_key_algorithm:'ssh-ed25519',host_key:key,chatgpt2api_port:33010};let deployCalls=0;
global._authFetch=async(url,options={})=>{let body={};if(url==='/api/extensions/targets')body={targets:[target]};else if(url==='/api/extensions/catalog')body={categories:[],items:[]};else if(url==='/api/extensions/targets/batch')body={target_ids:[]};else if(url==='/api/extensions/tasks')body={tasks:[]};else if(url==='/api/extensions/ssh/test')body={ok:true,host_key_algorithm:'ssh-ed25519',host_key:key,privileges:{is_root:true,can_deploy:true}};else if(url==='/api/extensions/deploy/plan')body={discovery:{environment:{},instances:[],deployment_modes:[]},plan:{id:'plan-one',ready:true,evidence_manifest:{complete:true,snapshot_digest:'a'.repeat(64)},operations:[],safety:[]}};else if(url==='/api/extensions/deploy'){deployCalls+=1;body={task_id:'must-not-start'}}return {ok:true,status:200,text:async()=>JSON.stringify(body)}};
eval(source);window.extensionLoadServices=async()=>{};await window.loadExtensions();window.extensionLoadTarget('saved');element('extPassword').value='session-only';element('extImage').value='registry.example/chatgpt2api@sha256:'+('a'.repeat(64));window.extensionCredentialChanged();await window.extensionTestSSH();window.extensionNext(2);await window.extensionCreatePlan();allowConfirmation=false;await window.extensionStartDeploy();
if(deployCalls!==0)throw new Error('cancelled deployment confirmation submitted a deploy request');if(element('extensionMessage').textContent!=='extensions.deploy_confirm_cancelled')throw new Error('cancelled deployment did not show a safe recovery message');if(element('extPlanPreview').classList.contains('hidden')||element('extDeployBtn').disabled)throw new Error('cancelled deployment discarded the reviewed plan');
})();
'''
    result = subprocess.run(["node", "-e", node, str(source)], text=True, capture_output=True)
    assert result.returncode == 0, result.stderr
