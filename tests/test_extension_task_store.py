...
const target={id:'saved',name:'Saved',host:'safe.example',port:22,username:'user',host_key_algorithm:'',host_key:'',chatgpt2api_port:33010};let pairingStarts=0,probes=0;
global._authFetch=async(url)=>{let body={};if(url==='/api/extensions/targets')body={targets:[target]};else if(url==='/api/extensions/catalog')body={categories:[],items:[]};else if(url==='/api/extensions/targets/batch')body={target_ids:[]};else if(url==='/api/extensions/tasks')body={tasks:[]};else if(url==='/api/extensions/ssh/host-key/pair/start'){pairingStarts+=1;body={pairing_id:'local-pairing',expires_at:Date.now()/1000+30,helper_command:'printf local-confirmation'}}else if(url==='/api/extensions/ssh/host-key/probe'){probes+=1;body={}}return {ok:true,text:async()=>JSON.stringify(body)}};
eval(source);window.extensionLoadServices=async()=>{};await window.loadExtensions();window.extensionLoadTarget('saved');
...
if(pairingStarts!==1||probes!==0)throw new Error('novice confirmation did not use pairing-only start');if(element('extHostKeyPairingCommand').value!=='printf local-confirmation')throw new Error('pairing helper did not render');if(element('extGuidePrimaryBtn').textContent!=='status.processing'||!element('extGuidePrimaryBtn').disabled)throw new Error('guide did not wait for the confirmation-code steps');if(!element('extHostKeyPairingCopyBtn').focused)throw new Error('confirmation flow did not focus copy');window.extensionCancelHostKeyPairing();if(element('extGuidePrimaryBtn').textContent!=='extensions.host_key_pairing_start'||element('extGuidePrimaryBtn').disabled)throw new Error('cancel did not restore the restart action');if(!element('extHostKeyPairingStartBtn').focused)throw new Error('cancel did not restore keyboard focus to restart');
})();
'''
    result = subprocess.run(["node", "-e", node, str(source)], text=True, capture_output=True)
    assert result.returncode == 0, result.stderr
