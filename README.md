...
> 觉得 GenBox 有帮助？欢迎点亮 [Star](https://github.com/liwei9745/GenBox/stargazers)，或在 [Issues](https://github.com/liwei9745/GenBox/issues) 中反馈问题。
>
...
- [使用路径](#使用路径)
- [功能说明](#功能说明)
- [chatgpt2api 与 GenBox](#chatgpt2api-与-genbox-的关系)
- [支持的模型与协议](#支持的模型与协议)
- [配置说明](#配置说明)
- [安全边界](#安全边界)
- [技术栈与项目结构](#技术栈与项目结构)
...
| 模式 | 适用场景 | 认证与访问 |
|---|---|---|
| 本地模式 | 个人电脑、首次体验 | 仅本机访问，默认不要求管理员密钥 |
| 生产模式 | NAS、VPS、局域网或远程访问 | 生成管理员密钥，必须配置 `ALLOWED_ORIGINS` |
| Docker | 可重复部署和长期运行 | 默认生产模式，配置与数据通过挂载持久化 |

### 常见问题

<details>
<summary>Windows 双击后立即退出</summary>

1. 在终端中运行 `GenBox.exe`，保留启动日志。
2. 检查 `8891` 端口是否被占用，需要时在 `.env` 修改 `GENBOX_PORT`。
3. 确认 Windows Defender 或防火墙没有阻止客户端。
4. 如果仍无法启动，在 Issue 中提供版本、操作系统与已脱敏的错误文本。
...
运行 `xattr -c GenBox-macOS`，或在“系统设置 → 隐私与安全性”中选择仍要打开。

</details>

<details>
<summary>Linux 或 VPS 提示没有执行权限</summary>
...
| 模块 | 当前能力 | 说明 |
|---|---|---|
| 图片生成 | 文生图、图生图、变体、超分辨率 | 支持单模型工作台和多模型并排对比 |
| 视频生成 | 文生视频、图生视频、关键帧 | 统一提示词、参数和任务监视视图 |
| 提示词辅助 | 优化、撤销、快捷词条 | 模型名、用户原文和密钥不参与翻译 |
| 任务监视 | 进度、分模型状态、运行日志 | 生成时自动展开，日常使用时保持收纳 |

### 媒体库与历史

- 统一管理本地图片和视频，支持搜索、筛选、排序和批量操作。
- 保留模型、提示词、时间和任务结果，方便重新使用已有提示词。
- 远程 Pull 导入使用内容哈希去重，避免重复保存同一份媒体。

### 扩展中心

- 保存 VPS 目标，执行 SSH 主机指纹确认和只读环境发现。
- 生成固定部署计划，引导建立隔离 chatgpt2api 实例与 Tailscale 私网。
- 管理已部署服务、访问地址与用户明确选择保存的本机加密凭证。
- 不接受浏览器任意 shell 命令，部署命令由后端固定适配器组装。
...
GenBox 是多 Provider 工作台，具体可用模型取决于你配置的服务：

| 类型 | 常见适配方式 | 示例 |
|---|---|---|
| OpenAI 兼容 | `/v1` 风格端点、自建网关 | GPT Image、DALL-E、Flux、Stable Diffusion 兼容服务 |
| Gemini | Google Gemini 或兼容网关 | Gemini Image / Flash 系列 |
| Qwen | 通义千问或兼容网关 | Qwen Image、Wanx 系列 |
| Agnes | Agnes 图片与视频端点 | Agnes Image / Video |
| 通用 LLM | OpenAI 兼容对话端点 | 提示词优化 |

Provider 和模型名称均由用户配置，README 不承诺第三方端点的持续可用性。

## 配置说明

### Provider 管理

- 每个 Provider 单独维护类型、端点、模型、API Key 与默认参数。
- 支持多 Key 轮询、错误冷却和可选 HTTP/SOCKS5 代理。
- 浏览器界面不会翻译或改写模型名、密钥和用户提示词。

### 常用环境变量

| 变量 | 用途 |
|---|---|
| `APP_MODE` | `dev` 为本地模式，`prod` 启用管理员认证 |
| `GENBOX_PORT` | 服务端口；桌面/Docker 默认 `8891`，源码开发默认 `8892` |
| `ADMIN_KEY` | 生产模式管理员密钥 |
| `ALLOWED_ORIGINS` | 允许访问 API 的 HTTPS 或私网源 |
| `GENBOX_IMAGE` | Docker 更新使用的 GHCR 镜像 |

完整示例见 [`.env.example`](.env.example) 和 [`.env.docker.example`](.env.docker.example)。
...
| 前端 | 原生 HTML / CSS / JavaScript，固定键中英文 i18n |
| 远程运维 | AsyncSSH、Docker Compose、Tailscale |
| 媒体与安全 | Pillow、SHA-256、cryptography / Fernet |
| 发布 | PyInstaller、GitHub Actions、GHCR、SHA-256 校验 |

```text
GenBox/
├─ main.py                  # FastAPI 入口和主要 API
├─ config.py                # 环境、Provider 与管理员认证
├─ providers/               # 模型适配器与 Key Pool
├─ extensions/              # 扩展中心、VPS、私网和凭证库
├─ sync/                    # 远程 Pull、Push 接收与去重基础
├─ static/                  # 单页 Web UI 和实验室
├─ tests/                   # 安全、扩展、同步与发布测试
├─ scripts/                 # 打包、冒烟检查与截图脱敏
└─ docs/                    # 产品、架构、路线图和状态文档
```
...
贡献前请先阅读 [AGENTS.md](AGENTS.md) 与 [开发生命周期](docs/DEVELOPMENT-LIFECYCLE.md)。提交 PR 时请：

1. 保持修改范围聚焦，不提交 `.env`、`storage/`、日志或用户媒体。
2. 更新与行为风险相匹配的测试。
3. 运行 `python -m pytest -q` 和相关 JavaScript 语法检查。
4. 明确区分已完成功能、设计基础和后续阶段。
...
|---|---|---|
| [yukkcat/chatgpt2api](https://github.com/yukkcat/chatgpt2api) | [yukkcat](https://github.com/yukkcat) | 当前扩展中心 chatgpt2api 部署、介绍和集成边界的上游参考 |
...
特别感谢 [@yukkcat](https://github.com/yukkcat) 在 [PR #4](https://github.com/liwei9745/GenBox/pull/4) 中提出 GHCR Docker Compose 发布包方案。v2.5.0 在此基础上补充了生产配置模板、客户端冒烟测试、校验和文档。
