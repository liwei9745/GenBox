...
> [!IMPORTANT]
> **v2.6.7 是当前稳定版本。** 它在精准改图 V4 基础上加入可折叠的本轮图库、跨会话工作流回看和恢复、日期/工作流筛选，以及更紧凑的底图、显示模式和更换图片控制。Provider 能力和支持尺寸仍须由端点明确声明或由用户确认；别名只会通过显式配置或用户确认的受限兼容映射继承能力，缺失或歧义时会安全拒绝。[查看本次更新](release-notes-v2.6.7-zh.md)
...
- ONNX 抠图 checkpoint 不随 v2.6.7 发布包提供，生产联网安装也处于禁用状态；没有经过验证的本地模型时，抠图按钮会安全保持不可用。
...
| 我想了解 | 从这里开始 |
|---|---|
| 安装、升级与常见问题 | [v2.6.7 发布说明](release-notes-v2.6.7-zh.md) · [更新记录](CHANGELOG.md) |
