...
> [!IMPORTANT]
> **v2.6.8：精准改图重大更新。** GPT 目标模型已通过用户人工验收，支持多尺寸扩图、可视化批注改图与本地人物抠图工作流；新增模型优先的尺寸联动、版本尺寸与时间显示，以及首次使用快速导航。[查看本次更新](release-notes-v2.6.8-zh.md)

> [!NOTE]
> 精准改图的远程生成由你配置的模型端点完成。本次 GPT 验收由用户完成，发布自动化不提交付费生成请求；其他厂商的改图联动尚待人工验收。发布包包含 NumPy、ONNX Runtime 和本地抠图适配器，但**不包含 ONNX 抠图 checkpoint**；生产联网下载/安装保持禁用，因为其来源、训练数据沿革和商业使用权仍为 **UNVERIFIED（未验证）**。操作者手动放置的模型也必须通过完整性与可执行性检查后才能启用。
