...
> [!IMPORTANT]
> **v2.6.9：精准改图兼容性修复。** 保留已验收的 GPT 批注改图、扩图和本地抠图成果，新增 Gemini / Nano Banana 接入与独立尺寸联动，修复中转响应重复解压和分辨率档位误选。Klong `nano-banana2` 大部分预设已由用户验收；个别极端画幅标记为实验性。[查看本次更新](release-notes-v2.6.9-zh.md)

> [!NOTE]
> 精准改图的远程生成由你配置的模型端点完成。GPT 及上述 Nano Banana 验收仅覆盖对应端点和已测尺寸，不代表所有同名模型、中转或比例均可用；发布自动化不提交付费生成请求。发布包包含 NumPy、ONNX Runtime 和本地抠图适配器，但**不包含 ONNX 抠图 checkpoint**；生产联网下载/安装保持禁用，因为其来源、训练数据沿革和商业使用权仍为 **UNVERIFIED（未验证）**。操作者手动放置的模型也必须通过完整性与可执行性检查后才能启用。
...
**先选模型，再选尺寸；用箭头指出位置，用方框或圆圈圈出区域，让修改意图更直观。**

- **智能扩图与画布适配**：支持下表的 1K、2K、4K 档尺寸预设，搭配可编辑的构图说明，完成横图、竖图、方图与超宽画幅扩展。切换模型端点或模型后，尺寸能力与授权状态随之更新。
- **可视化批注，指哪改哪**：用箭头、矩形（方框）、椭圆（圆圈）或画笔标注修改位置，再用文字说明想要的变化。支持批注移动、缩放、撤销和重做，让二次改图不再只靠长提示词描述位置。实际修改精度仍取决于模型。
- **本地一键抠出人物**：配置并验证兼容的本地模型后，一键分离人物与背景，并可继续进行边缘精修、羽化和前景恢复；抠图处理无需发送图片到线上生图服务。AI 消除仍使用所选在线模型。
- **连续精修与版本回看**：保留多轮改图与工作流历史，支持前后对比、恢复继续编辑，并显示所选版本的实际分辨率和版本时间。首次使用提供快速导航，随时可从“文档说明”重看。

### GPT 扩图尺寸预设

**USER-CONFIRMED（用户人工验收通过，2026-09-10）：** 本轮目标模型标识为 `gpt-image-2.5-c`。以下为 GenBox 当前预设表，不代表所有 GPT 服务、同名别名或其他厂商的官方支持清单。
...
> 档位是预设分类，实际像素以上表为准，部分比例经过像素对齐。“模型尺寸”严格校验模型实际输出；未声明尺寸可由用户对当前端点、模型和具体尺寸单独授权试用。“裁切适配”则在一次模型改图后进行本地裁切缩放。授权不会自动生成，失败不会自动重试；503 不等于尺寸不支持。Gemini、Qwen 等其他厂商将在后续分别人工验收。
...
- ONNX 抠图 checkpoint 不随 v2.6.8 发布包提供，生产联网安装也处于禁用状态；没有经过验证的本地模型时，抠图按钮会安全保持不可用。
...
发送端改动仍在独立分支和上游提案流程中；使用未经确认的原始 chatgpt2api 版本前，请先核对其是否实现 GenBox Push v1。源图删除默认关闭、开发环境禁用，并且只有在用户对本次操作明确选择删除、收到认证且 SHA-256 匹配的 `safe_to_delete_source=true` 回执、并再次确认源文件内容未变化时才允许执行。上游合并与真实环境行为不能从 GenBox v2.6.7 的发布成功中推断。
...
| 我想了解 | 从这里开始 |
|---|---|
| 安装、升级与常见问题 | [v2.6.8 发布说明](release-notes-v2.6.8-zh.md) · [更新记录](CHANGELOG.md) |
