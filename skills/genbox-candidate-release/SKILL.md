...
description: Audit and prepare a GenBox experimental or candidate client and Docker image without publishing, using local smoke tests, digest-to-commit evidence, secret scanning, and explicit phase-gate reporting.
---

# GenBox Candidate Release

Use this skill for a local `experimental` or `candidate` release rehearsal. It
does not authorize a stable release, a `latest` tag, a GitHub Release, or any
VPS operation.
...
1. Capture the exact Git commit and require a clean or explicitly scoped
   candidate diff. Do not emit a release tag or publish it.
...
4. Build a local candidate Docker image only when the Docker daemon is
   available. Record the immutable image ID/digest together with the source
   commit. Run an isolated Compose loopback probe with a generated placeholder
   administrator key that never enters Git or ordinary logs.
...
7. Inspect workflows for candidate-safe triggers/tags and capture a workflow
   run URL or mark GitHub Actions evidence `UNVERIFIED` when no authenticated
   query is available.
