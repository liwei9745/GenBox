(function (global) {
  ...
  var MESSAGES = {
    ...
    "video.first_frame":{"zh-CN":"首帧","en":"First frame"},
    "video.reference_short":{"zh-CN":"参考","en":"Reference"},
    "video.first_last":{"zh-CN":"首尾帧","en":"First and last frames"},
    "video.last_frame":{"zh-CN":"尾帧","en":"Last frame"},
    "video.upload":{"zh-CN":"拖拽图片或点击上传","en":"Drag an image or click to upload"},
    "video.add_image":{"zh-CN":"添加图片","en":"Add image"},
    "video.preview_image":{"zh-CN":"预览图片","en":"Preview image"},
    "video.remove_image":{"zh-CN":"移除图片","en":"Remove image"},
  ...
  MESSAGES['creator.precision_start_nav_hint'] = {"zh-CN":"按引导完成模型、尺寸和构图设置；点击任一项可定位操作区，不会提交任务。以后可从“文档说明”再次打开。","en":"Follow the model, size, and composition guidance. Click any item to locate its controls without submitting a task. Reopen this guide from Documentation anytime."};
  MESSAGES['creator.precision_start_model'] = {"zh-CN":"先选择模型端点，再在“显示模型”中勾选并确定编辑模型；接入方式默认自动，只有协议不匹配时才需要调整。显示“待确认”时，仅在确认支持改图后启用；没有端点请先到“模型设置”配置。","en":"Choose an endpoint, then select and confirm the editing model in Displayed models. Auto connection is recommended; adjust it only when the protocol does not match. Enable an unconfirmed model only if you know it supports editing. Add missing endpoints in Model settings."};
  MESSAGES['creator.precision_start_size'] = {"zh-CN":"改变尺寸时，先选尺寸方式，再选预设；模型尺寸会严格校验，裁切适配会在本地裁切或缩放。按提示授权试用后，选择构图说明并按需修改，加载底图即可生成。","en":"When resizing, choose the size mode and then a preset. Model size is strictly validated; Crop to fit uses local cropping or scaling. Authorize a trial when prompted, choose composition guidance, load a base image, and generate."};
  ...
  MESSAGES['creator.precision_size_geometry_warning'] = {"zh-CN":"实验性：已出现比例不匹配","en":"Experimental: aspect mismatch observed"};
  MESSAGES['creator.precision_size_output_limit_warning'] = {"zh-CN":"实验性：输出可能超过安全边界","en":"Experimental: output may exceed safety limits"};
